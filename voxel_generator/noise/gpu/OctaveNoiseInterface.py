...
def octave_noise_compute(world_x, world_z, x_np, z_np, octave_count, random): # FIXME take a list of x, z coordinates
    seed = 1
    ...
    kernel = KernelLoader([ 'PerlinNoise.cl', 'OctaveNoise.cl' ])
    program = kernel.build(context)

    r_g = cl.Buffer(context, mf.WRITE_ONLY, total_values * 8) # TOTAL_VALUES * NUMBER_OF_BYTES
    program.OctaveNoise(queue, (total_values,), None, numpy.int32(octave_count), p_g, x_g, z_g, r_g)
